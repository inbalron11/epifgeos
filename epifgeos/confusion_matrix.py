...
class Confusion_matrix:
    ...
    def __init__(self, true_raster_path, predicted_raster_path, numpprocesses, outpath, show=False):
        # open the raster
        self.true_raster = gdal.Open(true_raster_path)
        self.predicted_raster = gdal.Open(predicted_raster_path)
        self.outpath = outpath
        self.numpprocesses = numpprocesses

        # read raster as ndrarray and split it into n nd arrays (n = number of processes)
        self.true_array = self.true_raster.GetRasterBand(1).ReadAsArray()
        self.predicted_array = self.predicted_raster.GetRasterBand(1).ReadAsArray()
        self.splittrue = np.array_split(self.true_array, self.threads)
        self.splitpredicted = np.array_split(self.predicted_array, self.threads)

        # get raster lables (unique pixel values)
        self.lables = np.unique(self.true_array)[1::]
