...
class Rasterize:
    def __init__(self, shapefile_path, out_raster_path, reference_raster, out_raster_name='predicted_raster',
        ...
        self.shapefile_data_source = ogr.Open(shapefile_path, 1)
        self.reference = gdal.Open(reference_raster)
        self.source_layer = self.shapefile_data_source.GetLayer(0)
        self.srs = self.source_layer.GetSpatialRef()
        self.target_ds = gdal.GetDriverByName('GTiff').Create(out_raster_path + out_raster_name,
                                                              self.reference.RasterXSize,
                                                              self.reference.RasterYSize, 1, gdal.GDT_Byte)
        self.target_ds.SetGeoTransform(self.reference.GetGeoTransform())
        ...
        self.out_raster_name = out_raster_name
...
if __name__ == '__main__':
    shapepath = '/home/inbal/inbal/gdal/outputs/polygonize/thematic.shp'
    outpath = '/home/inbal/inbal/gdal/outputs/'
    ...
    #rasterize(shapepath,outpath,reference,out_raster_name='metulapred')
    ras = Rasterize(shapepath, outpath, reference, out_raster_name='metulapred')
